...
module.exports = (router) => {
  ...
  router.post(
    '/sessions',
    expressMiddlewareAdapter(authMiddlewareFactory('user')),
    expressRouterAdapter(createSessionControllerFactory())
  );
  router.patch(
    '/sessions/:session_id',
    expressMiddlewareAdapter(authMiddlewareFactory('user')),
    expressRouterAdapter(updateSessionControllerFactory())
  );
  router.delete(
    '/sessions/:session_id',
    expressMiddlewareAdapter(authMiddlewareFactory('user')),
    expressRouterAdapter(deleteSessionControllerFactory())
  );
};
